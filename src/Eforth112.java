...
public class Eforth112 {	// ooeforth
	...
	public static void setup_dic() {
		...
		for (String w: words) { dictionary.add(new Code(w)); }
		for (String w: immd)  {	dictionary.add(new Code(w).immediate()); }
	}
...
//		public List<Integer> qf = new ArrayList<>(Arrays.asList(-1)) ;
		...
		public final HashMap<String, Runnable> lookUp = new HashMap<>() {{
			// stacks
			...
			put( "'",     ()-> { 
				...
				for (var w:dictionary) {
					if (s.equals(w.name)) { 
						stack.push(w.token); found = true;break;
					}
				}
			...
			put( "(",     ()-> { tok.nextToken("\\)"); });
			put( ".(",    ()-> { System.out.print(tok.nextToken("\\)")); });
			put( "\\",    ()-> { tok.nextToken("\n"); });
