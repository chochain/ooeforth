...
public class Eforth201 {	// ooeforth
	static Scanner in;
	...
	static TextArea input = new TextArea("words");
	static TextArea output = new TextArea("ooeForth 2.01");
	static boolean compiling=false;
	static int base=10;
	static int wp,ip;

	private static void dict_setup() {
		final String prim[] = {
			":", "dup", "over", "4dup", "swap", "rot", "-rot", "2swap",
			"pick", "roll", "2dup", "2over", "drop", "nip", "2drop", 
			">r", "r>", "r@", 
			"+", "-", "*", "/", "mod", "*/", "*/mod",
			"and", "or", "xor", "negate", 
			"0=", "0<", "0>", "=", "<", ">", "<>", ">=", "<=",
			"base@", "base!", "hex", "decimal", "cr", ".", ".r", "u.r",
			"key", "emit", "space", "spaces", 
			"]", "[", "'", "exit", "exec", "create", "variable", "constant",
			"@", "!", "+!", "?", "array@", "array!", ",", "allot", 
			"does", "to", "is", 
			"here", "words", ".s", "see", "time", "ms"
		};
		final String immd[] = {
			";", "$\"", ".\"", "(", ".(", "\\",
			"if", "else", "then", 
		...
		for (var s: prim) { dictionary.add(new Code(s)); }
		for (var s: immd) { dictionary.add(new Code(s).immediate()); }
	}
	public static void main(String args[]) {	// ooeforth 2.01
		output.append("ooeForth2.01\n");
		// code dictionary 
		dict_setup();							// CC:
		// GetKeyChar
		...
		output.setPreferredSize(new Dimension(800, 700));
	...
	static class Code {									// one size fits all objects
		...
		public int token=0;
		public Code(String n) 			 { name=n; token=fence++; }				// CC:
		public Code(String n, boolean f) { name=n; token=0;       }				// CC: branch
		public Code(String n, int d) 	 { name=n; token=fence; qf.add(d);  }	// CC: dolit
		public Code(String n, String l)  { name=n; token=fence; literal=l;  }	// CC: dostr
	...
	static public HashMap<String, Consumer<Code>> lookUp=new HashMap<>() {{
		...
		put("emit",(c)->{output.append(Character.toString(stack.pop()));});
		...
		put("branch",(c)->{
			if(!(stack.pop()==0)) {for(var w:c.pf) w.xt();}			// true,  execute pf
			else {for(var w:c.pf1) w.xt();}							// false, execute pf1
			});
		put("if",(c)->{
			Code last=dictionary.get(dictionary.size()-1);
			last.addCode(new Code("branch", false));				// add branching node
			dictionary.add(new Code("temp", false));				// start collecting
			});
		put("else",(c)->{
			Code last=dictionary.get(dictionary.size()-2);
			Code temp=dictionary.get(dictionary.size()-1);
			last.pf.get(last.pf.size()-1).pf.addAll(temp.pf);		// populate if xxx then
			temp.pf.clear();
			last.pf.get(last.pf.size()-1).struct=1;});
		put("then",(c)->{
			Code last=dictionary.get(dictionary.size()-2);
			Code temp=dictionary.get(dictionary.size()-1);
			if (last.pf.get(last.pf.size()-1).struct==0) {			// populate if xxx then
				last.pf.get(last.pf.size()-1).pf.addAll(temp.pf);
				dictionary.remove(dictionary.size()-1);
			} else {
			last.pf.get(last.pf.size()-1).pf1.addAll(temp.pf);		// populate else yyy then
		...
		put("repeat",(c)->{
			...
			dictionary.remove(dictionary.size()-1);					// drop temp 
			});
		put("again",(c)->{
			Code last=dictionary.get(dictionary.size()-2);
			Code temp=dictionary.get(dictionary.size()-1);
		...
		put("words",(c)->{int i=0;for (var w:dictionary) {
			output.append(w.name + " ");i++;
			if (i>15) {output.append("\n");i=0;}}});
